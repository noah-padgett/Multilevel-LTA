...
compute_r2_stats <- function(K, alpha1, sig_alpha1, sig_alpha2, gamma, beta){
  ...
  # 6. total variance of C2
  total_c2 <- sum((pi ** 2) / 3 + split1 + tot + sig_alpha2)
  
  # 7. compute proportion of variance in C2 explained by a1
  R2_1 <- ex_a1 <- con_c1_on_c2 / total_c2
  
  # 8. proportion of variance in C2 explained by a2
  R2_2 <- ex_a2 <- sum(sig_alpha2 / total_c2)
  # 9. compute proportion of variance in C2 explained by residual of C1
...
compute_r2_stats(
  ...
  alpha2 = c(-15, -15),
  sig_alpha1 = c(0.8, 0.8),
  sig_alpha2 = c(0.25, 0.25),
  gamma = c(18, 17.5, 0, 16),
  beta = c(0.3, 0.3, 0, 0.3)
)
...
# # c1
# var_c1_2_c21 <- ((alpha2[1]+gamma[1])**2)*var_c1k[1] + ((alpha2[2]+gamma[3])**2)*var_c1k[2] - ((alpha2[1]+gamma[1])*(alpha2[2]+gamma[3]))*p1[1]*p1[2] 
# var_c1_2_c21
# # c2
# var_c1_2_c22 <- ((alpha2[1]+gamma[2])**2)*var_c1k[1] + ((alpha2[2]+gamma[4])**2)*var_c1k[2] - ((alpha2[1]+gamma[2])*(alpha2[2]+gamma[4]))*p1[1]*p1[2] 
# var_c1_2_c22
# # total
# var_c1_2_c2 <- ((alpha2[1]+gamma[1])**2 + (alpha2[1]+gamma[2])**2)*var_c1k[1] + ((alpha2[2]+gamma[3])**2+(alpha2[2]+gamma[4])**2)*var_c1k[2] - ((alpha2[1]+gamma[1])*(alpha2[2]+gamma[3]) + (alpha2[1]+gamma[2])*(alpha2[2]+gamma[4]))*p1[1]*p1[2] 
# var_c1_2_c2
# 
# 
# # 4. Split forwarded variance by icc
# var_c1_exp <- 
#   icc[1]*(( (alpha2[1]+gamma[1])**2 + (alpha2[1]+gamma[2])**2)*var_c1k[1] -
#             ((alpha2[1]+gamma[1])*(alpha2[2]+gamma[3]))*p1[1]*p1[2]) +
#   icc[2]*(( (alpha2[2]+gamma[3])**2+(alpha2[2]+gamma[4])**2)*var_c1k[2] - 
#             ((alpha2[1]+gamma[2])*(alpha2[1]+gamma[4]))*p1[1]*p1[2])
# 
# var_c1_res <- 
#   (1-icc[1])*(( (alpha2[1]+gamma[1])**2 + (alpha2[1]+gamma[2])**2)*var_c1k[1] -
#             ((alpha2[1]+gamma[1])*(alpha2[2]+gamma[3]))*p1[1]*p1[2]) +
#   (1-icc[2])*(( (alpha2[2]+gamma[3])**2+(alpha2[2]+gamma[4])**2)*var_c1k[2] - 
#             ((alpha2[1]+gamma[2])*(alpha2[1]+gamma[4]))*p1[1]*p1[2])
# var_c1_exp; var_c1_res; var_c1_exp+ var_c1_res
# 
